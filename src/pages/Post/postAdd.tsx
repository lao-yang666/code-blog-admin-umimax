...
const { postControllerGetPostById: getPostDetail,
  postControllerCreateDraft: addDraft, postControllerUpdateDraft: modifyDraft } =
...
const PostAdd: React.FC<unknown> = () => {
  ...
  const mdInfoRef = useRef<{ title: string, content: string, lastContent: string }>
    ({ title, content: mdValue, lastContent: mdValue });
    ...
    <div>
      <Div>
        <Input
          size='large'
          value={title}
          onChange={(e) => { handleChangeTitle(e.target.value) }} placeholder="请输入标题" maxLength={80} />
        <ButtonBox>
