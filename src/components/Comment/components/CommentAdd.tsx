...
import { Button, Dropdown, Flex, Mentions, Row } from 'antd';
// 添加或删除评论 组件 
import data from '@emoji-mart/data'
import Picker from '@emoji-mart/react'
const CommentAdd: React.FC<{ addComment: Function }> = (props) => {
  ...
  return (
    <div className="comment-send">
      <div className="user-face">
        <img className="user-head" alt="" />
      </div>
      <div className="textarea-container">
        <Mentions value={value} onChange={setValue} allowClear rows={4} maxLength={500} placeholder='文明发言,友好交流'/>
        <Flex className='comment-bottom-action' align='center' justify='space-between'>
          <Dropdown dropdownRender={() => <Picker data={data} locale='zh' onEmojiSelect={onEmojiSelect} />} trigger={['click']} >
          ...
          <Button disabled={!value} type='primary' onClick={handleSubmit}>发送</Button>
        </Flex>
      </div>
